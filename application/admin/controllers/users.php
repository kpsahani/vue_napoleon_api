...
class users extends CI_Controller { 
	...
	function index()
	{
        $this->data['users'] = $this->common->get_data_all('paytm');
        // print_r($this->data['users']);die;

        $this->data['total'] = count($this->data['users']);
        $this->load->view('users/userslist', $this->data);
	}

	function add()
	{
		$this->load->view('users/signup1',$this->data);
	}
	...
	function insertdata()
	{
        ...
        if (isset($_FILES['avtar']['name']) && $_FILES['avtar']['name'] != null)
        {
            ...
            $config['file_name'] = str_replace(' ', '_', $this->input->post('username')) . '_PRO_' . time();
