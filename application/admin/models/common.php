...
Class Common extends CI_Model {
    ...
    function select_database_id($tablename, $columnname, $columnid)
    {   
        ...
        $query = $this->db->get_where($tablename, array($columnname => $columnid));
   ...
   // check unique avaliblity
    function check_unique_avalibility_where($table,$colpass,$passvalue)
    {   
        // $this->db->select("*");
        // $this->db->from($table);
       
        if($passvalue != ''){
            // $this->db->where($colemail ."!=",$adminemail);
            // $q = $this->db->get();
            $query = $this->db->get_where($table, array($colpass => $passvalue));
            // print"hiii";die;
            return $query->result_array();
        }
